...
export default function BOQDetail() {
  ...
  const handleConvertToInvoice = async () => {
    ...
    try {
      ...
      toast({
        title: "BOQ Converted",
        description: `New invoice ${invoiceNumber} has been created from this BOQ.`,
      });
      ...
      toast({
        title: "Error",
        description: "Failed to convert BOQ to invoice.",
        variant: "destructive",
      });
    } finally {
  ...
  if (!doc) return <div className="p-20 text-center text-muted-foreground">BOQ not found.</div>;
            ...
            <CardContent>
              <div className="space-y-3">
                {doc.attachments && doc.attachments.length > 0 ? (
                  ...
                  <p className="text-xs text-muted-foreground text-center py-4">No supporting files.</p>
                )}
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="border-none shadow-lg bg-card/50">
            ...
            <CardContent className="space-y-4">
              <div className="flex flex-col gap-2">
                ...
                <span className="font-bold uppercase tracking-wider text-indigo-400">{doc.status}</span>
              </div>
              <div className="flex flex-col gap-2">
