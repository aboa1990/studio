...
import { Document, CompanyProfile, Client, LibraryDocument, LineItem, Attachment } from './types';
...
export const saveDocument = async (docData: Partial<Document>) => {
  ...
  const docRef = doc(db, COMPANIES_COLLECTION, activeId, 'documents', id);
  await setDoc(docRef, { ...docData, id, profileId: activeId }, { merge: true });
  return { ...docData, id, profileId: activeId } as Document;
};
